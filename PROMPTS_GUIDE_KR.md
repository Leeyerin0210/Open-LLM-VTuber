...
## 6. 입력 처리 및 형식
### `chat_format_prompt` (`chat_format_prompt.txt`)
- **역할**: 사용자 입력 메시지의 형식을 AI에게 설명합니다.
- **내용**:
    - 입력이 `[닉네임]: 메시지` 형태로 들어옴을 인지시킵니다.
    - `[Unknown]` 닉네임이나, `[닉네임]:` 형식이 없는 단순 메시지는 **개발자(Developer)**가 직접 말하는 것이므로, 특별히 예의를 갖추고 주의를 기울이도록 지시합니다.
    - 개발자의 발언은 AI에게 하는 말일 수도, 시청자에게 하는 말일 수도 있으므로 문맥을 파악하여 적절히 반응하도록 안내합니다.
